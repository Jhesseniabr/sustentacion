...
// Verificar que el número de registro esté presente en la solicitud GET
if (isset($_GET['numero_registro'])) {
    $numero_registro = $_GET['numero_registro'];

    // Buscar los datos del ticket con el número de registro proporcionado
    $sql_ticket = "SELECT * FROM ticket WHERE numero_registro = '$numero_registro'";
    $query_ticket = mysqli_query($con, $sql_ticket);
    $ticket = mysqli_fetch_assoc($query_ticket);
    ...
    if ($ticket) {
        ...
        // Buscar los datos del cliente asociado al ticket
        ...
        if ($cliente) {
            ...
            $pdf->MultiCell(0, 5, iconv("UTF-8", "ISO-8859-1", "Fecha: " . date("d/m/Y h:i a")), 0, 'C', false);
            ...
            echo "No se encontró el cliente asociado al ticket.";
        }
    } else {
        echo "No se encontró el ticket con el número de registro proporcionado.";
    }
} else {
    echo "No se proporcionó el número de registro del ticket.";
}
?>
